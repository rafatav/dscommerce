...
public class ControllerExceptionHandler {
    ...
    public ResponseEntity<CustomError> requestNotFound(RequestNotFound e, HttpServletRequest request) {
        HttpStatus status = HttpStatus.NOT_FOUND;
        CustomError err = new CustomError(Instant.now(), status.value(), e.getMessage(), request.getRequestURI());
        return ResponseEntity.status(status).body(err);
    }

    @ExceptionHandler(DatabaseNotFound.class)
    public ResponseEntity<CustomError> databaseNotFound(DatabaseNotFound e, HttpServletRequest request) {
